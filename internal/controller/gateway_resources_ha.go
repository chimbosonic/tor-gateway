...
// frontendTorrcContent is the torrc for the frontend tor daemon. It is a
// vanilla tor with a loopback ControlPort and cookie auth; HiddenService
// directives are absent because onionbalance manages them via the control port.
// DataDirectory and CookieAuthFile both point at a subdirectory of the
// mount, not the mount root: Tor refuses a DataDirectory not owned by its
// uid, and emptyDir mount roots stay root-owned even with FSGroup. Tor
// creates the subdir itself owned by 65532 the first time it runs. This
// mirrors the Mode A torrc shape (see names.go: dataMountPath / torDataDir).
const frontendTorrcContent = `# generated by tor-gateway operator — do not edit by hand
SocksPort 0
ControlPort 9051
CookieAuthentication 1
CookieAuthFile /var/lib/tor/data/control_auth_cookie
...
// BuildFrontendTorrcConfigMap renders the torrc ConfigMap for the frontend tor
// container. No HiddenService directives — onionbalance drives those via the
// control port.
func BuildFrontendTorrcConfigMap(gw *gwv1.Gateway, scheme *runtime.Scheme) (*corev1.ConfigMap, error) {
	cm := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
		...
		Data: map[string]string{"torrc": frontendTorrcContent},
	}
	if err := controllerutil.SetControllerReference(gw, cm, scheme); err != nil {
		return nil, err
	}
